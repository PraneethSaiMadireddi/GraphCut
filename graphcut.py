...
class GraphCut(object):
    ...
    def update_init_value_seams(self, corner_overlap, corner, mask_seam):
        for i in range(self.patchRows):
            for j in range(self.patchCols):
                x_crt = corner[0] + i
                y_crt = corner[1] + j
                if (corner_overlap[0] <= x_crt < corner_overlap[0] + self.overlapRows and corner_overlap[1] <= y_crt <
                        corner_overlap[1] + self.overlapCols):
    ...
    def init(self):
        t = [0, 0]
        self.old[t[0]:t[0] + self.patchRows, t[1]:t[1] + self.patchCols] = self.texture[0:self.patchRows,
                                                                           0:self.patchCols]
    ...
    def num_neighbors_in_mask(self, p):
        ...
        for i in range(-1, 2):
            for j in range(-1, 2):
                if p[0] + i >= 0 and p[0] < self.imRows and 0 <= p[1] + j < self.imCols:
        ...
        for u in range(corner[0] - 1, corner[0] + self.overlapRows):
            for v in range(corner[1] - 1, corner[1] + self.overlapCols):
                if self.overlap_zone[u][v] == 1:
                    if self.num_neighbors_in_overlap([u, v]) >= 1:
    ...
    def find_edge_cost(self, x_crt, y_crt, x_adj, y_adj, A, B):
    ...
    def find_minimum_cut_required(self, t):

        nb_pixels = [0]
        self.new[t[0]:t[0] + self.patchRows, t[1]:t[1] + self.patchCols] = self.texture[0:self.patchRows,
                                                                           0:self.patchCols]
            ...
            for j in range(self.overlapCols):
                ...
                if self.mask[x_crt][y_crt] == 1:
                    ...
                    if self.seams[x_crt][y_crt][0] != 0:
                        if self.seams[x_crt][y_crt][1] == 2 or self.seams[x_crt][y_crt][1] == 3:
                            if self.overlap_zone[x_crt][y_crt] == 1 and self.mask[x_crt + 1][y_crt] == 1:
                                ...
                                color1 = abs(
                                    self.texture[s_As[0]][s_As[1]][0] - self.texture[s_At[0]][s_At[1]][0]) + abs(
                                    self.texture[t_As[0]][t_As[1]][0] - self.texture[t_At[0]][t_At[1]][0])
                                color2 = abs(
                                    self.texture[s_As[0]][s_As[1]][1] - self.texture[s_At[0]][s_At[1]][1]) + abs(
                                    self.texture[t_As[0]][t_As[1]][1] - self.texture[t_At[0]][t_At[1]][1])
                                color3 = abs(
                                    self.texture[s_As[0]][s_As[1]][2] - self.texture[s_At[0]][s_At[1]][2]) + abs(
                                    self.texture[t_As[0]][t_As[1]][2] - self.texture[t_At[0]][t_At[1]][2])
                                cost = (color1 + color2 + color3) / 3
                                g.add_edge(0, nb_pixels[0] + 1 + seam_supp, capacity=cost)

                                color1 = abs(self.texture[s_As[0]][s_As[1]][0] - self.new[x_crt][y_crt][0]) + abs(
                                    self.texture[t_As[0]][t_As[1]][0] - self.new[x_crt + 1][y_crt][0])
                                color2 = abs(self.texture[s_As[0]][s_As[1]][1] - self.new[x_crt][y_crt][1]) + abs(
                                    self.texture[t_As[0]][t_As[1]][1] - self.new[x_crt + 1][y_crt][1])
                                color3 = abs(self.texture[s_As[0]][s_As[1]][2] - self.new[x_crt][y_crt][2]) + abs(
                                    self.texture[t_As[0]][t_As[1]][2] - self.new[x_crt + 1][y_crt][2])
                                cost = (color1 + color2 + color3) / 3
                                g.add_edge(mat_num[i][j], nb_pixels[0] + 1 + seam_supp, capacity=cost)

                                color1 = abs(self.new[x_crt][y_crt][0] - self.texture[s_At[0]][s_At[1]][0]) + abs(
                                    self.new[x_crt + 1][y_crt][0] - self.texture[t_At[0]][t_At[1]][0])
                                color2 = abs(self.new[x_crt][y_crt][1] - self.texture[s_At[0]][s_At[1]][1]) + abs(
                                    self.new[x_crt + 1][y_crt][1] - self.texture[t_At[0]][t_At[1]][1])
                                color3 = abs(self.new[x_crt][y_crt][2] - self.texture[s_At[0]][s_At[1]][2]) + abs(
                                    self.new[x_crt + 1][y_crt][2] - self.texture[t_At[0]][t_At[1]][2])
                                cost = (color1 + color2 + color3) / 3
                        ...
                        if self.seams[x_crt][y_crt][1] == 4 or self.seams[x_crt][y_crt][1] == 3:
                            if self.overlap_zone[x_crt][y_crt] == 1 and self.mask[x_crt][y_crt + 1] == 1:
                                ...
                                color1 = abs(self.texture[s_As[0]][s_As[1]][0] - self.new[x_crt][y_crt][0]) + abs(
                                    self.texture[t_As[0]][t_As[1]][0] - self.new[x_crt][y_crt + 1][0])
                                color2 = abs(self.texture[s_As[0]][s_As[1]][1] - self.new[x_crt][y_crt][1]) + abs(
                                    self.texture[t_As[0]][t_As[1]][1] - self.new[x_crt][y_crt + 1][1])
                                color3 = abs(self.texture[s_As[0]][s_As[1]][2] - self.new[x_crt][y_crt][2]) + abs(
                                    self.texture[t_As[0]][t_As[1]][2] - self.new[x_crt][y_crt + 1][2])
                                cost = (color1 + color2 + color3) / 3
                                g.add_edge(mat_num[i][j], nb_pixels[0] + 1 + seam_supp, capacity=cost)

                                color1 = abs(self.new[x_crt][y_crt][0] - self.texture[s_At[0]][s_At[1]][0]) + abs(
                                    self.new[x_crt][y_crt + 1][0] - self.texture[t_At[0]][t_At[1]][0])
                                color2 = abs(self.new[x_crt][y_crt][1] - self.texture[s_At[0]][s_At[1]][1]) + abs(
                                    self.new[x_crt][y_crt + 1][1] - self.texture[t_At[0]][t_At[1]][1])
                                color3 = abs(self.new[x_crt][y_crt][2] - self.texture[s_At[0]][s_At[1]][2]) + abs(
                                    self.new[x_crt][y_crt + 1][2] - self.texture[t_At[0]][t_At[1]][2])
                                cost = (color1 + color2 + color3) / 3
                    ...
                    if i < self.overlapRows - 1 and self.mask[x_crt + 1][y_crt] == 1 and down == False:
                        x_adj = x_crt + 1
                        y_adj = y_crt
                        cost = self.find_edge_cost(x_crt, y_crt, x_adj, y_adj, self.old, self.new)
